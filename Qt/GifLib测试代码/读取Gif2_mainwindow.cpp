    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->btnDecode2,&QPushButton::clicked,[=]{
        gifToImage2(QString(qApp->applicationDirPath()+"/test2.gif"));
    });
...
{
    //没释放
    GifByteType *red_buffer = new GifByteType[300*300];
    GifByteType *green_buffer = new GifByteType[300*300];
    GifByteType *blue_buffer = new GifByteType[300*300];
    //GifByteType *out_buffer = new GifByteType[300*300];
    ...
    for(int i = 0; i < imgpaths.count(); i++) {
        ...
        if ((color_map = GifMakeMapObject(map_size, NULL)) == NULL){
        ...
        if (GifQuantizeBuffer(300, 300, &map_size,
                              ...
                              out_buffer, color_map->Colors) == GIF_ERROR)
        {
            qDebug()<<"error";
            return;
        }

        qDebug()<<"MakeSavedImage"<<i;
        ...
        GraphicsControlBlock *gcb = new GraphicsControlBlock;

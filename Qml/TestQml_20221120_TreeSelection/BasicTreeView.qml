...
//修改自Controls 1.x TreeView
//文档参见TreeView文档
//龚建波 2022-11-20
//1.增加双击展开/收起
//2.收起时，currentIndex不自动到下一行，而是回到父节点
//备注：
//view和style这一对共轭父子互相依赖，所以两个都得重写
//view和item loader亦如是
//使用时需要设置style，放在内部会循环引用
BasicTableView {
    id: root
    ...
    function expandIndex(index) {
        if (index.valid && index.parent)
            expandIndex(index.parent)
        expand(index)
    }
    ...
    __itemDelegateLoader: BasicTreeViewItemLoader {

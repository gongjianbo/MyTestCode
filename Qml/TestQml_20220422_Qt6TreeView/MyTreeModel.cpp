...
QModelIndex MyTreeModel::parent(const QModelIndex &index) const
{
    ...
    auto parentWeakPtr = childItem->parentItem;
    if (!parentWeakPtr) {
        return QModelIndex();
    }
    auto parentPtr = parentWeakPtr.toStrongRef();
...
void MyTreeModel::resetItems()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        lv1->parentItem = rootItem.toWeakRef();
